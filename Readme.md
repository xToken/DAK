...
Once downloaded, you would want to merge them with your ns2/lua folder (there will not be any files overridden).  From there, you will want to edit Server.lua and add the line `Script.Load("lua/DAKLoader_Server.lua")` 
...
AutoConcede Plugin
* This plugin adds an Automatic Concedede check to the GameEnd function, which will alert players if the teams are different by a configurable amount of players
