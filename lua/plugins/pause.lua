...
local function OnPluginInitialized()
	...
	originalNS2PlayingTeamUpdateResourceTowers = Class_ReplaceMethod("PlayingTeam", "UpdateResourceTowers", 
		function(self)
			...
			if DAK:GetTournamentMode() and gamepaused then
				return true
			end
...
local function PausedJoinTeam(self, player, newTeamNumber, force)
	if DAK:GetTournamentMode() and gamepaused and (newTeamNumber ~= 1 and newTeamNumber ~= 2) then
		return true
	end
end
...
local function UpdateMoveState(deltatime)
	...
	if gamepaused then
		...
		if gamepausedreadymessagetime + DAK.config.pause.kPausedReadyNotificationDelay < Shared.GetTime() then
		...
		DAK:DisplayMessageToAllClients("PauseResumeMessage", gamepausedpausingteam, (DAK.config.pause.kPauseMaxPauses - (ConditionalValue(gamepausedpausingteam == 1, gamepausedstates.team1pauses, gamepausedstates.team2pauses))))
...
local function UpdateServerPauseState()
	...
	if gamepausedinittime - tt > 0 then
		DAK:DisplayMessageToAllClients("PauseWarningMessage", ConditionalValue(gamepaused, "resume", "pause"), (gamepausedinittime - tt))
	else
		...
		if player ~= nil and not gamepaused then
			...
			if teamnumber and ValidateTeamNumber(teamnumber) then
				...
				if teamnumber == 1 then
					if gamepausedstates.team1pauses < DAK.config.pause.kPauseMaxPauses then
				...
				if validpause then
					gamepausedstates.team1resume = false
					gamepausedstates.team2resume = false
					gamepausedinittime = Shared.GetTime() + DAK.config.pause.kPauseChangeDelay
